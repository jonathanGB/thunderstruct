...
def datdot(A, B):
    ...
    if len(B.shape) == 1:
        ...
        now1 = time.time()
        res = A.dot(B)
        datdot.restimes.append(time.time() - now1)
        now1 = time.time()
        #res = Math.dot(A,B)
        datdot.res2times.append(time.time() - now1)
        datdot.vectime += time.time() - now1
        return res
    else:
        datdot.matcount += 1
        res = A.dot(B)
        datdot.mattime += time.time() - now
        return A.dot(B)
    return res
...
def gen_arc(b, leader=None, eta=2, also=False, max_n=1000, h=1, mg=False, method='iccg', mg_args={}):
    b = b.copy()
...
def get_mg_arrays(bound_shape, levels=3, v_up=2, v_down=0, scale=4, method='iccg'):
